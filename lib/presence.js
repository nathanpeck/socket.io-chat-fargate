...
Presence.prototype.list = function(returnPresent) {
  ...
  this.client.hgetall('presence', function(err, results) {
    if (err) {
      console.error('Failed to get presence from Redis: ' + err);
      return returnPresent([]);
    }

    var presence = Object.values(results);

    for (var present of presence) {
      var details = JSON.parse(present);
...
/**
  ...
  * @param
**/
Presence.prototype._clean = function(results) {
